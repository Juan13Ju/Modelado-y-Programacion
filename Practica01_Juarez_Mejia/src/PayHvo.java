class PayHvo implements PayStrategy{
    ...
    public void cobrar(Cliente cli, Servicio ser, int mesActual){
        ...
        if(mesActual - mesInicial < 3){
            System.out.println(cli.getNombre() + " paga $0 " + " por la tarifa inicial de " + ser.nombre);
            return;
        }else{

            // Si ya paso el periodo de 3 meses, entonces desontamos 150 cada mes
            System.out.println(cli.getNombre() + " paga $" + cantidad + " por la tarifa unica de " + ser.nombre);
            cli.setDinero(cli.getDinero() - 130);
        }
